...
@Component(value = "MOB_SwipeRightUntilElementIsVisible")
public class MOB_SwipeRightUntilElementIsVisible implements Nlp {
	...
	public NlpResponseModel execute(NlpRequestModel nlpRequestModel) throws NlpException {
		...
		try {
			...
			int fromX = (int) (screenSize.getWidth()*0.7); // Get x location
			int toX = (int)(screenSize.getWidth()*0.2);
			...
			if (count >= maxSwipeCount) {
				...
				nlpResponseModel.setMessage("Swiped right  until element option is visible");
			}
		}
		catch(Exception e) {
			nlpResponseModel.setStatus(CommonConstants.fail);
			nlpResponseModel.setMessage("Failed to swipe right until element is visible");
		}
